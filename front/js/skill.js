...
function showSkills(name_skill, area_skill, level_skill) {
    ...
    if (!!name_skill && !!area_skill) {
        ...
        if (times <= 10) {
            var newItem = "<span id=" + id + " class='btn-level level-" + level_skill + " skill-tag'><input type='hidden' name='level-" + id + "' value='" + level_skill + "'><form id='form-" + id + "' method='post' action=''><input type='hidden' name='skill-" + id + "' value='" + name_skill + "'></form><input type='hidden' name='area-" + id + "' value='" + area_skill + "'> <i class='fa-sharp fa-solid fa-xmark' onclick='deleteSkill(" + id + ")'></i> " + name_skill + "</span>";
            $("#skill_list").append(newItem);
